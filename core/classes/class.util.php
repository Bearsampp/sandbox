...
class Util
{
    ...
    /**
     * Deletes files or symlinks named 'current' from specified start paths, excluding any within the 'core' directory.
     *
     * This function iterates over each provided start path, and recursively searches for files or symlinks named 'current'.
     * It skips over any directories named 'core' and their contents. Each found item named 'current' is then attempted to be
     * deleted, and logs are generated for both successful and failed deletion attempts.
     *
     * @param   array  $startPaths  An array of directory paths from which to start the search and deletion process.
     */
    public static function deleteCurrentFiles(array $startPaths)
    {
        global $bearsamppRoot;

        // Ensure $startPaths is an array
        if ( !is_array( $startPaths ) ) {
            self::logError( 'Expected an array of start paths' );
        ...
        foreach ( $startPaths as $startPath ) {
            ...
            $resolvedStartPath = realpath( $startPath );

            // Check if the directory exists
            if ( !$resolvedStartPath || !is_dir( $resolvedStartPath ) ) {
                self::logError( 'The directory does not exist: ' . $startPath );
                continue;
            }

            $dirIterator = new RecursiveDirectoryIterator( $resolvedStartPath, RecursiveDirectoryIterator::SKIP_DOTS );
            $iterator    = new RecursiveIteratorIterator( $dirIterator, RecursiveIteratorIterator::SELF_FIRST );

            foreach ($iterator as $item) {
                // Skip the 'core' directory and all its children
                if (strpos($item->getPathname(), $resolvedStartPath . '/core') === 0) {
                    $iterator->skipChildren();
                    continue;
                }

                // Skip the 'temp' directory and all its children
                if (strpos($item->getPathname(), $resolvedStartPath . '/temp') === 0) {
                    $iterator->skipChildren();
                    continue;
                }

                // Check if the item is a directory or symlink named 'current'
                if ($item->getFilename() === 'current' && ($item->isDir())) {
                    // Attempt to delete the directory or symlink
                    if (@Batch::removeSymlink($item->getPathname())) {
                        self::logInfo('Deleted: ' . $item->getPathname());
                    } else {
                        self::logError('Failed to delete: ' . $item->getPathname());
                    }
                }
            }
        }
    ...
    /**
     * Clears all 'current' symlinks from the specified directories.
     *
     * This method retrieves an array of paths where 'current' symlinks are expected to be found,
     * and then deletes these symlinks to ensure the directories are clean from any such links.
     * It utilizes the `createCurrentPathsArray` method from the `Root` class to get the paths,
     * and the `deleteCurrentFiles` method from the `Util` class to perform the deletion.
     */
    public static function clearCurrentSymlinks()
    {
        global $bearsamppRoot;

        // Get array for files and symlinks named "current"
        $currentPaths = [
        ...
        // Delete all the 'current' symlinks
        self::deleteCurrentFiles($currentPaths);
    }
}
