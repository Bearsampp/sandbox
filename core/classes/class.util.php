...
class Util
{
    ...
    public static function cleanArgv(int|string $name, string $type = 'text'): mixed
    {
     ...
     *
     * @return bool True on success, false on failure or if the file doesn't exist.
     */
    public static function disableLaunchStartup()
    {
        ...
        // File doesn't exist, so we can consider it "successfully" disabled
        return true;
    }
    ...
    /**
     * Safely converts a value to hexadecimal.
     * Ensures the input is properly cast to integer to avoid dechex() deprecation warnings.
     * Handles null values by defaulting to 0.
     *
     * @param   mixed  $value  The value to convert to hexadecimal.
     *
     * @return string The hexadecimal representation of the value.
     */
    public static function toHex($value): string
    {
        ...
        // Ensure value is an integer to avoid dechex() deprecation warnings
        if (!is_int($value)) {
            if (is_numeric($value)) {
                $value = (int)$value;
            } else {
                self::logWarning("Non-numeric value passed to toHex(): " . gettype($value));
                $value = 0;
            }
        }
    ...
    /**
     * Safely converts a hexadecimal value to decimal.
     *
    ...
    {
        // Check if string is null or empty before using substr
     ...
     *
     * This improved implementation uses non-blocking sockets for reliable port checking
     * and supports both IPv4 and IPv6 connections. It properly handles connection errors
     * and timeouts to avoid hanging when ports are in a transitional state.
     *
     * @param   mixed   $port       The port number to check.
     ...
     */
    public static function isPortInUse($port, string $host = 'localhost', bool $checkBoth = false, $timeout = 1): mixed
    {
        // Ensure port is properly validated and converted to integer
        if (!is_numeric($port) || $port <= 0 || $port > 65535) {
            self::logError("Invalid port number: " . $port);
            return false;
        }
        
        // Explicitly cast port to integer to avoid type issues
        $port = (int)$port;
        
        // Log that we're checking this port
        self::logDebug("Checking if port $port is in use on $host...");
        
        // TCP check
        $inUse = false;
        $startTime = microtime(true);

        // Use non-blocking socket with timeout for safer operation
        if (function_exists('socket_create') && function_exists('socket_connect')) {
            // Try with IPv4 socket extension (more reliable)
            ...
            if ($socket !== false) {
                ...
                // Try to connect - initial attempt will likely return false due to non-blocking
                @socket_connect($socket, $host, $port);
                
                // Check socket status with timeout handling
                $read = $write = array($socket);
                $except = null;
                
                // Wait for writability which indicates connection success
                // or wait until timeout (shorter timeout to prevent long hangs)
                if (socket_select($read, $write, $except, min($timeout, 2)) > 0) {
                    // Check if socket is actually connected
                    $sockStatus = @socket_get_option($socket, SOL_SOCKET, SO_ERROR);
                    if ($sockStatus === 0) {
                        $inUse = true;
                        self::logDebug("Port $port is in use (socket connect succeeded)");
                    }
                }
            ...
            // If still not found and checkBoth is true, try UDP with similar non-blocking approach
            if (!$inUse && $checkBoth && (microtime(true) - $startTime) < $timeout) {
                $socket = @socket_create(AF_INET, SOCK_DGRAM, SOL_UDP);
                if ($socket !== false) {
                    // For UDP, we try to bind
                    $result = @socket_bind($socket, '0.0.0.0', $port);
                    if (!$result) {
                        $inUse = true;
                        self::logDebug("Port $port is in use (UDP bind failed)");
                    }
                    @socket_close($socket);
                }
            }
        ...
        // Fallback to fsockopen if socket functions aren't available
            ...
            // Set an error handler to catch fsockopen warnings silently
            set_error_handler(function() { return true; });
            $connection = @fsockopen($host, $port, $errno, $errstr, min($timeout, 1));
            restore_error_handler();
            
            if (is_resource($connection)) {
                $inUse = true;
                self::logDebug("Port $port is in use (fsockopen succeeded)");
                fclose($connection);
            }
        }

        // If the port is in use, get the process information with timeout protection
        if ($inUse) {
            // Only attempt process identification if we have enough time left
            $elapsedTime = microtime(true) - $startTime;
            $remainingTime = $timeout - $elapsedTime;
            
            // Skip process identification if we're running out of time
            if ($remainingTime < 0.5) {
                self::logWarning("Skipping process identification for port $port - approaching timeout");
                return 'N/A';
            }
            
            try {
                // Set a function timeout
                $oldTimeLimit = ini_get('max_execution_time');
                set_time_limit(10); // Hard 10-second limit for getProcessUsingPort
                
                // Use a separate try-catch to ensure we restore the time limit
                try {
                    self::logDebug("Attempting to identify process using port $port");
                    $process = Batch::getProcessUsingPort($port);
                    self::logDebug("Process using port $port: " . ($process ?? 'N/A'));
                } catch (Exception $e) {
                    self::logWarning("Exception while identifying process for port $port: " . $e->getMessage());
                    $process = null;
                }
                ...
                self::logError("Critical error in process identification for port $port: " . $e->getMessage());
                return 'N/A';
            }
        }

        self::logDebug("Port $port is not in use");
        return false;
    }
    ...
    public static function removeService($service, $name)
    {
        ...
        // Special handling for Memcached service
        if ($name == 'Memcached' || (method_exists($service, 'getName') && $service->getName() == 'bearsamppmemcached')) {
            self::logInfo('Using special handling for Memcached service removal');
            ...
            if ($service->isRunning()) {
                self::logWarning('Could not stop Memcached service normally, attempting to kill process');
                $exe = $service->getBinPath();
                if (!empty($exe)) {
                    Win32Ps::killProcessByExe($exe);
                    self::logInfo(sprintf('Killed Memcached process using executable path: %s', $exe));
                }
                
                // Also try direct taskkill
                exec('taskkill /F /IM memcached.exe /T 2>nul');
                self::logInfo('Executed taskkill command for memcached.exe');
            }
            ...
            if ($service->isInstalled()) {
                ...
                if ($service->delete()) {
                    self::logInfo('Memcached service successfully removed');
                    return true;
                }
                
                // If normal deletion fails, try SC command
                self::logWarning('Normal service deletion failed, trying SC command');
                $scCmd = 'sc delete ' . $service->getName();
                exec($scCmd);
                
                // Check if service is gone
                sleep(1);
                if (!$service->isInstalled()) {
                    self::logInfo('Memcached service removed using SC command');
                    return true;
                }
                
                // If SC command fails, try registry cleanup
                self::logWarning('SC command failed, trying registry cleanup');
                $regCmd = 'reg delete "HKLM\\SYSTEM\\CurrentControlSet\\Services\\' . $service->getName() . '" /f';
                exec($regCmd);
                
                // Final check
                sleep(1);
                if (!$service->isInstalled()) {
                    self::logInfo('Memcached service removed using registry cleanup');
                    return true;
                }
                
                self::logError('Failed to remove Memcached service after all attempts');
                return false;
            } else {
                self::logWarning('Memcached service does not exist');
                return true;
            }
        }
        ...
        if ($service->isInstalled()) {
            ...
            if ($service->isRunning() && !$service->stop()) {
                // Fallback to process termination if service won't stop normally
                self::logWarning(sprintf('Could not stop %s service normally, attempting to kill process', $name));
                $exe = $service->getBinPath();
                if (!empty($exe)) {
                    Win32Ps::killProcessByExe($exe);
                    self::logInfo(sprintf('Killed %s process using executable path: %s', $name, $exe));
                }
            }
    ...
    public static function startService($bin, $syntaxCheckCmd, $showWindow = false)
    {
        ...
        // If service is already running, try to stop it first
        if ($service->isRunning()) {
            self::logInfo(sprintf('Service %s is already running, attempting to restart', $name));
            if (!$service->stop()) {
                // Fallback to process termination if service won't stop normally
                self::logWarning(sprintf('Could not stop %s service normally, attempting to kill process', $name));
                $exe = $service->getBinPath();
