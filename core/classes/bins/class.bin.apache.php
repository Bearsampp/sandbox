...
class BinApache extends Module
{
	...
	protected function updateConfig($version = null, $sub = 0, $showWindow = false)
	{
		...
		Util::replaceInFile($conf, array(
			...
			'/^#?LoadModule\sphp.*/'                      => ($bearsamppBins->getPhp()->isEnable() ? '' : '#') . 'LoadModule ' . $apachePhpModuleName . ' "' . Path::getModuleSymlinkPath(
					$bearsamppBins->getPhp()
				) . '/' . $apachePhpModuleDll . '"',
			'/^#?LoadModule\sphp_*/'                      => ($bearsamppBins->getPhp()->isEnable() ? '' : '#') . 'LoadModule ' . $apachePhpModuleName . ' "' . Path::getModuleSymlinkPath(
					$bearsamppBins->getPhp()
				) . '/' . $apachePhpModuleDll . '"',
