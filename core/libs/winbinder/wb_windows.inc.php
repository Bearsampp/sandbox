...
function wb_create_control($parent, $class, $caption = '', $xpos = 0, $ypos = 0, $width = 0, $height = 0, $id = null, $style = 0, $lparam = null, $ntab = 0)
{
    switch ($class) {
        case Accel:
            return wb_accel_table($parent, $caption);
    ...
    switch ($class) {
        case ListView:        // Array with items to be checked
            ...
            foreach ($values as $index) {
                wb_listview_item_checked($ctrl, $index, 1);
            }
            break;
        ...
        case TreeView:        // Array with items to be checked
            ...
            return wb_treeview_item_value($ctrl, $item, $value);

        default:

            if ($value !== null) {
                return wb_value($ctrl, $value, $item);
            }
    }
}
...
function wb_set_text($ctrl, $text, $item = null, $subitem = null)
{
    ...
    switch (wb_get_class($ctrl)) {
        case ListView:
            ...
            if ($item !== null) {
                if (!is_array($text) && $subitem !== null) {
                    ...
                    wb_listview_item_text($ctrl, $item, $subitem, $text);
                } else {
                    ...
                    for ($sub = 0; $sub < count($text); $sub++) {
                        if ($text) {
                            if (($text[$sub] !== null)) {
                                wb_listview_item_text($ctrl, $item, $sub, (string)$text[$sub]);
                            }
                        } else {
                            wb_listview_item_text($ctrl, $item, $sub, '');
                        }
                    }
                }
            } else {
        ...
        case TreeView:
            ...
            if ($item) {
                return wb_treeview_item_text($ctrl, $item, $text);
            } else {
        ...
        default:
            ...
            // removed when wb_text() accepts numeric types correctly
            if (is_array($text)) {
                return wb_text($ctrl, $text, $item);
            } else {
                return wb_text($ctrl, (string)$text, $item);
            }
    }
}
...
function wb_set_selected($ctrl, $selitems, $selected = true)
{
    switch (wb_get_class($ctrl)) {
        ...
        case Menu:
            return wb_menu_item_checked($ctrl, $selitems, $selected);
        ...
        case TreeView:
            wb_treeview_item_selected($ctrl, $selitems);
            break;
    ...
    switch (wb_get_class($ctrl)) {
        case ListView:
            ...
            for ($i = 0; $i < count($items); $i++) {
                ...
                wb_listview_item_text($ctrl, -1, 0, (string)$items[$i][0]);
                ...
                for ($sub = 0; $sub < count($items[$i]) - 1; $sub++) {
                    if ($param) {
                        ...
                        wb_listview_item_text($ctrl, $last, $sub + 1, $result);
                    } else {
                        wb_listview_item_text($ctrl, $last, $sub + 1, (string)$items[$i][$sub + 1]);
                    }
                }
            }
